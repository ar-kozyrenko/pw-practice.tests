...
test('CS-0003 Add-remove an item', 
  ...
  await page.locator('[data-test="Espresso"]').click();
  await page.locator('[aria-label="Cart page"]').click();
  await page.locator('[data-v-8965af83][aria-label="Add one Espresso"]').click()
  await expect(page.locator('#app')).toContainText('$10.00 x 2');  
  await page.locator('[data-v-8965af83][aria-label="Remove one Espresso"]').click()
...
test('CS-0006 Payment Details popup - the name and email values are displayed', 
  ...
  await page.locator('[data-test="Espresso"]').click();
  await page.locator('[data-test="checkout"]').click();
  await page.locator('[id="name"]').fill('test1');
  await page.locator('[id="email"]').fill('test@mail.com');
  await expect(page.locator('[id="name"]')).toHaveValue('test1')
  await expect(page.locator('[id="email"]')).toHaveValue('test@mail.com');
});
